...
- 统一问答与历史记录，可选知识库筛选，支持关键词 / 向量混合检索、DeepSeek 解读和原文定位。
- DeepSeek 先给结论，再分条说明；模型选择证据片段编号，系统回填原文引用。资料不足明确回答“无法确认”。
- 回答结构或引用错误最多修复一次；仍失败则提示具体原因，原文折叠展示，不冒充总结答案。引用准确不等于结论一定正确。
